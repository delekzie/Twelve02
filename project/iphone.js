...
allProducts.map((info, key)=> {
    show.innerHTML +=`
   <div class="box2 bg-white rounded-md drop-shadow-lg max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg">
  <div class="flex justify-center bg-gray-100 h-32">
    <img id="productImage" src="${info.image}" alt="" class="object-contain h-full w-auto">
  </div>
  <div class="mx-2">
    <h1 class="mt-4 text-sm font-semibold break-words" id="productTitle">
      Name: ${info.name}
    </h1>
    <h1 class="mt-2 text-xs font-light" id="productPrice">
      Price: $ ${info.price}
    </h1>
    ...
    <div class="flex justify-center mt-8">
      <button
        class="border border-ash px-4 py-2 text-xs rounded-md font-semibold text-white bg-DullBlue hover:bg-blue-700"
        onclick="addCart(${key})"
      >
        ADD TO CART
      </button>
